...
function PhotosView({ savedImages, onPhotoClick, onDeleteImage }: { savedImages: Array<{ id: number; imageUrl: string; text: string; savedDate: Date }>; onPhotoClick: (image: { id: number; imageUrl: string; text: string; savedDate: Date }) => void; onDeleteImage: (id: number) => void }) {
  // 이미지를 년/월로 그룹화
          ...
          <div className="grid grid-cols-2 gap-[8px]">
            {groupedImages[yearMonth].map((image) => (
              <div key={image.id} className="relative group">
                <div className="relative h-[168px] rounded-[10px] w-full overflow-hidden bg-[#E0E0E0] cursor-pointer hover:opacity-80 transition-opacity" onClick={() => onPhotoClick(image)}>
                  <img
                    src={image.imageUrl}
                    alt="저장된 이미지"
...
export default function CollectionPage({
  ...
  isLoggedIn: boolean;
  onPhotoClick: (image: { id: number; imageUrl: string; text: string; savedDate: Date }) => void;
}) {
  ...
  if (!isLoggedIn) {
    return (
      <div className="flex justify-center items-start min-h-[100dvh] bg-[#fff9f0] w-full overflow-x-hidden overflow-y-scroll">
        <div className="bg-[#fff9f0] relative w-full min-h-[100dvh] pb-[68px]" style={{ touchAction: "pan-y" }}>
          {/* 상단 헤더 */}
  ...
  return (
    <div className="flex justify-center items-start min-h-[100dvh] bg-[#fff9f0] w-full overflow-x-hidden overflow-y-scroll">
      <div className="bg-[#fff9f0] relative w-full min-h-[100dvh] pb-[68px]" style={{ touchAction: "pan-y" }}>
        ...
        {/* 컨텐츠 */}
        {activeTab === "photo" ? <PhotosView savedImages={savedImages} onPhotoClick={onPhotoClick} onDeleteImage={onDeleteImage} /> : <QuotesView savedQuotes={savedQuotes} onDeleteQuote={onDeleteQuote} />}
