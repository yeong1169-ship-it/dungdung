...
// 기본 명언 리스트 (localStorage에 데이터가 없을 때 사용)
const defaultQuotes = [
  {
...
function Frame28() {
  return (
    <div className="absolute left-0 right-0 top-[1449px] w-full aspect-[430/827]" data-name="Frame28">
      {/* 배경 이미지와 그라데이션 그룹 */}
...
function Frame10({ onClick, isActive }: { onClick: () => void; isActive: boolean }) {
  return (
    <button onClick={onClick} className="content-stretch flex flex-col gap-[3px] items-center relative shrink-0 w-[47px] cursor-pointer">
      ...
      <p className={`font-['Pretendard',sans-serif] leading-[15px] min-w-full not-italic relative shrink-0 text-[10px] text-center w-[min-content] ${isActive ? 'text-[#8faf3e]' : 'text-[#5c4033]'}`} style={{ fontWeight: 400 }}>Home</p>
    </button>
  );
}
...
function Frame9({ onClick, isActive }: { onClick: () => void; isActive: boolean }) {
  return (
    <button onClick={onClick} className="content-stretch flex flex-col gap-[3px] items-center relative shrink-0 w-[47px] cursor-pointer">
      ...
      <p className={`font-['Pretendard',sans-serif] leading-[15px] min-w-full not-italic relative shrink-0 text-[10px] w-[min-content] ${isActive ? 'text-[#8faf3e]' : 'text-[#5c4033]'}`} style={{ fontWeight: 400 }}>Download</p>
    </button>
  );
}
...
function Frame8({ onClick, isActive }: { onClick: () => void; isActive: boolean }) {
  return (
    <button onClick={onClick} className="content-stretch flex flex-col gap-[3px] items-center relative shrink-0 w-[47px] cursor-pointer">
      ...
      <p className={`font-['Pretendard',sans-serif] leading-[15px] min-w-full not-italic relative shrink-0 text-[10px] text-center w-[min-content] ${isActive ? 'text-[#8faf3e]' : 'text-[#5c4033]'}`} style={{ fontWeight: 400 }}>Talk</p>
    </button>
  );
}
...
function Frame7({ onClick, isActive }: { onClick: () => void; isActive: boolean }) {
  return (
    <button onClick={onClick} className="content-stretch flex flex-col gap-[3px] items-center relative shrink-0 w-[47px] cursor-pointer">
      ...
      <p className={`font-['Pretendard',sans-serif] leading-[15px] min-w-full not-italic relative shrink-0 text-[10px] w-[min-content] ${isActive ? 'text-[#8faf3e]' : 'text-[#5c4033]'}`} style={{ fontWeight: 400 }}>Collection</p>
    </button>
  );
}
...
function Frame6({ onClick, isActive }: { onClick: () => void; isActive: boolean }) {
  return (
    <button onClick={onClick} className="content-stretch flex flex-col gap-[3px] items-center relative shrink-0 w-[47px] cursor-pointer">
      <Icon6 />
      <p className={`font-['Pretendard',sans-serif] leading-[15px] min-w-full not-italic relative shrink-0 text-[10px] text-center w-[min-content] ${isActive ? 'text-[#8faf3e]' : 'text-[#5c4033]'}`} style={{ fontWeight: 400 }}>Shop</p>
    </button>
  );
}
...
export default function App() {
  ...
  const [savedImages, setSavedImages] = useState<Array<{ id: number; imageUrl: string; text: string; savedDate: Date }>>([]);
  ...
  const [selectedImage, setSelectedImage] = useState<{ id: number; imageUrl: string; text: string; savedDate: Date } | null>(null);
  ...
  // localStorage에서 저장된 컬렉션 로드
  useEffect(() => {
    // 저장된 명언 로드
    const savedCollectionQuotes = localStorage.getItem('saved_quotes');
    if (savedCollectionQuotes) {
      try {
        const parsed = JSON.parse(savedCollectionQuotes);
        if (Array.isArray(parsed)) {
          setSavedQuotes(parsed);
        }
      } catch (error) {
        console.error('저장된 명언 로드 실패:', error);
      }
    }

    // 저장된 이미지 로드
    const savedCollectionImages = localStorage.getItem('saved_images');
    if (savedCollectionImages) {
      try {
        const parsed = JSON.parse(savedCollectionImages);
        if (Array.isArray(parsed)) {
          // savedDate를 Date 객체로 변환
          const imagesWithDate = parsed.map(img => ({
            ...img,
            savedDate: new Date(img.savedDate)
          }));
          setSavedImages(imagesWithDate);
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (!quotes.length || !quotes[currentQuoteIndex]) return;
  ...
  }, [currentQuoteIndex, savedQuotes, quotes]);
  ...
  // 좋아요 버튼 클릭
  const handleLike = () => {
    if (!currentQuote) return; // 명언이 없으면 동작하지 않음
    ...
    if (!isLiked) {
      ...
      const updatedQuotes = savedQuotes.filter(q => q.quote !== quoteText);
      setSavedQuotes(updatedQuotes);
      // localStorage에 저장
      localStorage.setItem('saved_quotes', JSON.stringify(updatedQuotes));
  ...
  const handleNavigate = (page: "home" | "download" | "talk" | "shop" | "collection" | "downloadDetail" | "photoDetail") => {
    // 컬렉션 페이지로 이동하려고 할 때 로그인 체크
    if (page === "collection" && !isLoggedIn) {
      setIsLoginModalOpen(true);
      return;
    }
    setCurrentPage(page);
  ...
  if (currentPage === "downloadDetail") {
    return <DownloadDetail wallpaperId={selectedWallpaperId} onBack={() => setCurrentPage("download")} />;
  }

  // 다운로드 페이지면 Download 컴포넌트 렌더링
  if (currentPage === "download") {
    return <Download onNavigate={(page, wallpaperId) => {
      if (page === "downloadDetail" && wallpaperId) {
  ...
  return (
    <>
      ...
      <div className="flex justify-center items-start min-h-[100dvh] bg-[#fff9f0] w-full overflow-x-hidden overflow-y-auto">
        <div className="bg-[#fff9f0] relative w-full h-[3638px] pb-[68px] overflow-x-hidden" data-name="홈" style={{ margin: '0 auto', touchAction: 'pan-y' }}>
          {/* 하단 고정 네비게이션 */}
          <div className="bg-[rgba(255,249,240,0.91)] content-stretch flex flex-col h-[68px] items-center justify-center w-full z-50 backdrop-blur-sm" data-name="BottomNav" style={{ position: 'fixed', bottom: 0, left: 0, right: 0 }}>
            ...
            <Frame11 onNavigate={handleNavigate} currentPage={currentPage} />
          </div>
          ...
          {currentQuote && <Frame4 quote={currentQuote} onRefresh={handleRefreshQuote} onLike={handleLike} isLiked={isLiked} setIsLiked={setIsLiked} isLoggedIn={isLoggedIn} />}
          <DungsilInfo />
          ...
          <p className="absolute font-['Hakgyoansim_Dunggeunmiso_OTF:B',sans-serif] font-bold leading-[24px] left-1/2 not-italic text-[#8faf3e] text-[24px] text-center text-nowrap top-[2290px] translate-x-[-50%] whitespace-pre" style={{ fontWeight: 700 }}>둥실이의 대표 성격 3가지</p>
          <Component />
